...
module.exports = {
    ...
    get_pathway_data: function(collection_name, user_id) {
        return new Promise((resolve, reject) => {
            mongo.connect(db_url, {useUnifiedTopology: true}, function (err, db) {
                ...
                dbo.collection(collection_name).find({ creator_id: user_id }).toArray(function (err, res) {
                    if (err)
                        throw err;
                    if (res == undefined)
    ...
    get_pathway_lists: function(user_id) {
        return new Promise((resolve, reject) => {
            mongo.connect(db_url, {useUnifiedTopology:true, useNewUrlParser:true}, function(err, db) {
                if(err) reject(err)
                let user = db.db("users")
            })
        })
    }
}